...
var weatherBot = new TwitterPackage(secret);

weatherBot.stream('statuses/filter', {track: '@makers_twit_bot'}, function(stream) {
  ...
  stream.on('data', function(tweet) {
    ...
    if (splitArray.includes('weather') || splitArray.includes('temperature')) {
      ...
      request("http://api.openweathermap.org/data/2.5/weather?q=" + city + "&units=metric&APPID=134d393fd68fd2c3c4db178a50b5ddb8", function(error2,response2,body2) {
        ...
        weatherBot.post('statuses/update', statusObj,  function(error, tweetReply, response){
          if(error){
            console.log(error);
          }
